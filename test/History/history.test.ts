...
import { OrteliusDijetsTx } from '@/Explorer/ortelius/types';
...
jest.mock('@/Network/utils', () => {
    return {
        getDjtxAssetID: jest.fn().mockImplementation(() => {
...
describe('Import Tx', () => {
    it('single utxo import, all belong to wallet. P to X', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ImportTransaction);
        ...
        if (isHistoryImportExportTx(summary)) {
            ...
            expect(summary.source).toEqual('M');
            expect(summary.destination).toEqual('V');
    ...
    it('multiple import utxos, P to X', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ImportTx1);
        const myAddresses = [
...
describe('Export Tx', () => {
    it('multiple utxo export to C chain, all belong to wallet', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ExportTx);
        const myAddresses = [
        ...
        expect(summary.source).toEqual('V');
        expect(summary.destination).toEqual('U');
        expect(summary.fee.toString()).toEqual('1000000');
        expect(summary.amount.toString()).toEqual('10000001000000');
    });

    it('multiple utxos, P to X, with change utxo', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ExportTx1);
        const myAddresses = [
    ...
    it('single utxo, X to C', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ExportTx2);
        const myAddresses = [
    ...
    it('export C to X, single utxo', async () => {
        let data: OrteliusDijetsTx = JSON.parse(ExportTx3);
        ...
        expect(summary.type).toEqual('export');
        expect(summary.source).toEqual('U');
        expect(summary.destination).toEqual('V');
        expect(summary.fee.toString()).toEqual('1000000');
...
describe('Base Transaction Receive', () => {
    it('Simple receive DJTX, faucet drip', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTx);
    ...
    it('Receive single ANT token', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTx1);
    ...
    it('Receive 2 ANT tokens', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTx2);
...
describe('Base Transaction Send', () => {
    it('Simple send DJTX', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTxSend0);
    ...
    it('Send 1000 DJTX', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTxSend1);
        const myAddresses = [
    ...
    it('Send 2 ANTs', async () => {
        let data: OrteliusDijetsTx = JSON.parse(BaseTxSend2);
        const myAddresses = [
            'X-fuji1lcx7plqaajp6730hh7pvyk9l3urjm6auxc3fx5',
            'X-fuji1ysxw3923qapq43pukde9vud4wn84exyzdqletm',
...
describe('Staking TX', () => {
    it('Add validator 100 DJTX, staking finished, not rewarded', async () => {
        let data: OrteliusDijetsTx = JSON.parse(StakeTx0);
        const myAddresses = [
            'P-fuji1ur873jhz9qnaqv5qthk5sn3e8nj3e0kmafyxut',
    ...
    it('Add delegator 100 DJTX, uses lockedstakeable UTXOs, stake finished, rewarded', async () => {
        let data: OrteliusDijetsTx = JSON.parse(StakeTx1);
        const myAddresses = [
            'P-fuji1ur873jhz9qnaqv5qthk5sn3e8nj3e0kmafyxut',
    ...
    it('Fee received from delegator 0.000225438 DJTX', async () => {
        let data: OrteliusDijetsTx = JSON.parse(StakeTx2);
    ...
    it('Add validator, stake finished, rewarded', async () => {
        let data: OrteliusDijetsTx = JSON.parse(StakeTx3);
        const myAddresses = [
            'P-fuji1lzrmf3crp62ayj2kqwh9mzc5es9xv7q6cm00vl',
            'P-fuji1ur873jhz9qnaqv5qthk5sn3e8nj3e0kmafyxut',
