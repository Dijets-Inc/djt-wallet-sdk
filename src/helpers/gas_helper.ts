...
import { EVMInput, ExportTx, SECPTransferOutput, TransferableOutput, UnsignedTx } from 'dijets/dist/apis/evm';
import { ExportChainsC } from '@/Wallet/types';
import { bintools } from '@/common';
import { chainIdFromAlias } from '@/Network';
import { costExportTx, costImportTx } from 'dijets/dist/utils';
...
export function estimateExportGasFeeFromMockTx(
    ...
    const DJTX_ID = activeNetwork.djtxID;
    const djtxIDBuff = bintools.cb58Decode(DJTX_ID);

    const txIn = new EVMInput(from, amount, djtxIDBuff);
    const secpOut = new SECPTransferOutput(amount, [toBuff]);
    const txOut = new TransferableOutput(djtxIDBuff, secpOut);
 ...
 * @param amount The amount to export. In nDJTX.
