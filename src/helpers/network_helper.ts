...
/**
 * Given the base url of an Dijets API, requests the Network ID
 * @param url The base url for the Dijets API
 */
export async function getNetworkIdFromURL(url: string): Promise<number> {
    // TODO: Not be the best to assume /ext/info but Dijetsjs complicates things
...
export function createDijetsProvider(config: NetworkConfig) {
    return new Dijets(config.apiIp, config.apiPort, config.apiProtocol, config.networkID);
}
...
export async function canUseCredentials(config: NetworkConfig): Promise<boolean> {
    let provider = createDijetsProvider(config);
