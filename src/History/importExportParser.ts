...
import { bnToDjtxX, strip0x } from '@/utils';
import { getOutputsOfChain, getOutputTotals, getOwnedOutputs } from '@/Explorer/ortelius/utxoUtils';
import { findDestinationChain, findSourceChain, OrteliusDijetsTx } from '@/Explorer';
import { BN } from 'dijets';

export function getImportSummary(tx: OrteliusDijetsTx, addresses: string[], evmAddr: string): iHistoryImportExport {
...
export function getExportSummary(tx: OrteliusDijetsTx, addresses: string[]): iHistoryImportExport {
    let sourceChain = findSourceChain(tx);
    let chainAliasFrom = idToChainAlias(sourceChain);
