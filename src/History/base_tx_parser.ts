...
import { AVMConstants } from 'dijets/dist/apis/avm';
...
import { getDjtxAssetID } from '@/Network/utils';
import { OrteliusDijetsTx, OrteliusUTXO } from '@/Explorer';

export async function getBaseTxSummary(tx: OrteliusDijetsTx, ownerAddrs: string[]): Promise<iHistoryBaseTx> {
...
function getBaseTxNFTLosses(tx: OrteliusDijetsTx, ownerAddrs: string[]): iHistoryBaseTxNFTsSentRaw {
    let ins = tx.inputs || [];
...
function getBaseTxNFTGains(tx: OrteliusDijetsTx, ownerAddrs: string[]): iHistoryBaseTxNFTsReceivedRaw {
    let outs = tx.outputs || [];
...
async function getBaseTxTokensSummary(
    ...
    for (let i = 0; i < assetIDs.length; i++) {
        ...
        // If we sent djtx, deduct the fee
        if (id === getDjtxAssetID() && !tokenLost.isZero()) {
