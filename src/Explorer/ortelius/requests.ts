...
import { OrteliusDijetsTx, OrteliusEvmTx } from '@/Explorer';
 ...
 * @param txID
 */
export async function getTx(txID: string): Promise<OrteliusDijetsTx> {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }

    let url = `v2/transactions/${txID}`;
    return await explorer_api.get<OrteliusDijetsTx>(url);
}
 ...
 * @param endTime
 */
async function getTransactionsDijets(
    addresses: string[],
    limit = 20,
    chainID: string,
    endTime?: string
): Promise<OrteliusDijetsTx[]> {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
    ...
    const res = await explorer_api.post<{ transactions: OrteliusDijetsTx[]; next?: string }>(rootUrl, req);
...
export async function getAddressHistory(
    ...
    const promises = addrsChunks.map((chunk) => {
        return getTransactionsDijets(chunk, limit, chainID, endTime);
    });
