import { OrteliusDijetsTx } from '@/Explorer';
 ...
 * @param txs
 */
export function filterDuplicateOrtelius(txs: OrteliusDijetsTx[]) {
    let txsIds: string[] = [];
    let filtered: OrteliusDijetsTx[] = [];
 ...
 */
export function findSourceChain(tx: OrteliusDijetsTx): string {
 ...
 * @param tx Tx data from the explorer.
 */
export function findDestinationChain(tx: OrteliusDijetsTx): string {
    let baseChain = tx.chainID;
    let outs = tx.outputs || [];
...
export function getStakeAmount(tx: OrteliusDijetsTx): BN {
    let outs = tx.outputs || [];
