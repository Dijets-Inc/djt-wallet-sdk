...
import { bnToBig, bnToBigDjtxC, bnToBigDjtxX } from '@/utils';
    ...
    txs.map((tx) => {
        ...
        if (isHistoryBaseTx(tx)) {
            const tokenRows = tx.tokens.map((token) => {
                ...
                return [tx.id, dateStr, tx.type, token.asset.symbol, amtStr, `"${token.addresses.join('\r')}"`, 'V'];
            });
            rows.push(...tokenRows);
        } else if (isHistoryImportExportTx(tx)) {
            const amtStr = bnToBigDjtxX(tx.amount).toString();
            rows.push([tx.id, dateStr, tx.type, 'DJTX', amtStr, '', `${tx.source} to ${tx.destination}`]);
        } else if (isHistoryEVMTx(tx)) {
            const amtStr = bnToBigDjtxC(tx.amount).toString();
            const amtSigned = tx.isSender ? `-${amtStr}` : amtStr;
            if (!tx.input) {
                const addr = tx.isSender ? tx.to : tx.from;
                rows.push([tx.id, dateStr, tx.type, 'DJTX', amtSigned, addr, `C`]);
            }
        }
    });
    return rows;
