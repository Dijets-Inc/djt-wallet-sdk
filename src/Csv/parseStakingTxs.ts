...
import { bnToBigDjtxP } from '@/utils';
...
export function parseStakingTxs(txs: iHistoryStaking[]) {
    return txs.map((tx) => {
        ...
        const stakeAmt = bnToBigDjtxP(tx.amount).toString();
        ...
        if (tx.stakeEnd.getTime() > now) {
            ...
            rewardAmt = !tx.rewardAmount.isZero() ? bnToBigDjtxP(tx.rewardAmount).toString() : 'Not Reward Owner';
        } else {
