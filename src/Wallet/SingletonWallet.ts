...
import { KeyChain as AVMKeyChain, UnsignedTx as AVMUnsignedTx, Tx as AVMTx } from 'dijets/dist/apis/avm';
import {
...
import { Buffer as BufferDijets } from 'dijets';
import { EvmWallet } from '@/Wallet/EVM/EvmWallet';
import { UnsignedTx, Tx } from 'dijets/dist/apis/evm';
...
export class SingletonWallet extends WalletProvider implements UnsafeWallet {
    ...
    key = '';
    keyBuff: BufferDijets;
    evmWallet: EvmWallet;

    /**
     *
     * @param privateKey An dijets private key, starts with `PrivateKey-`
     */
    ...
    static fromEvmKey(key: string): SingletonWallet {
        let keyBuff = bintools.cb58Encode(BufferDijets.from(key, 'hex'));
