...
import { Credential, SigIdx, Signature } from 'dijets/dist/common';
import {
...
import { activeNetwork, dijets, web3 } from '@/Network/network';
import { Buffer } from 'dijets';
...
import { getAccountPathDijets, getAccountPathEVM } from '@/Wallet/helpers/derivationHelper';
import { PublicMnemonicWallet } from '@/Wallet/PublicMnemonicWallet';
import { getAppDjtx, getAppEth, getEthAddressKeyFromAccountKey, getLedgerConfigDjtx } from '@/Wallet/Ledger/utils';
...
export class LedgerWallet extends PublicMnemonicWallet {
    ...
    static async setTransport(transport: Transport) {
        ...
        // Update the config
        const config = await getLedgerConfigDjtx(transport);
    ...
    static async fromTransport(transport: Transport, accountIndex = 0) {
        ...
        const pubDjtx = await LedgerWallet.getExtendedPublicKeyDjtxAccount(transport, accountIndex);
        const pubEth = await LedgerWallet.getExtendedPublicKeyEthAddress(transport, accountIndex);

        let config = await getLedgerConfigDjtx(transport);
        ...
        const wallet = new LedgerWallet(pubDjtx, pubEth, accountIndex);
        return wallet;
    }
     ...
     */
    static async getExtendedPublicKeyDjtxAccount(transport: Transport, accountIndex = 0): Promise<string> {
        const app = getAppDjtx(transport);

        let res = await app.getWalletExtendedPublicKey(getAccountPathDijets(accountIndex));
    ...
    /**
     * Get information about the DJTX app on the ledger device.
     * @param transport
     */
    static async getDjtxConfig(transport: Transport) {
        return getLedgerConfigDjtx(transport);
    }
    ...
    async getTransactionPaths<UnsignedTx extends AVMUnsignedTx | PlatformUnsignedTx>(
        ...
        chainId: ChainIdType
    ): Promise<{ paths: string[]; isDjtxOnly: boolean }> {
    ...
    async getPathFromAddress(address: string) {
        ...
        let platformAddrs = await this.externalScan.getAllAddresses('M');
    ...
    async signX(unsignedTx: AVMUnsignedTx): Promise<AVMTx> {
        let tx = unsignedTx.getTransaction();
        let txType = tx.getTxType();
        let chainId: ChainIdType = 'V';

        let parseableTxs = ParseableAvmTxEnum;
        let { paths, isDjtxOnly } = await this.getTransactionPaths<AVMUnsignedTx>(unsignedTx, chainId);
    ...
    >(unsignedTx: UnsignedTx, paths: string[], chainId: ChainIdType): Promise<SignedTx> {
        ...
        const appDjtx = getAppDjtx(LedgerWallet.transport);
        const accountPath =
            chainId === 'U'
                ? bippath.fromString(`${ETH_ACCOUNT_PATH}`)
                : bippath.fromString(getAccountPathDijets(this.accountIndex));
        let txbuff = unsignedTx.toBuffer();
        let changePath = this.getChangeBipPath(unsignedTx, chainId);

        let ledgerSignedTx = await appDjtx.signTransaction(accountPath, bip32Paths, txbuff, changePath);
    ...
    async signHash(accountPath: any, bip32Paths: any, hash: Buffer): Promise<Map<string, Buffer>> {
        if (!LedgerWallet.transport) throw ERR_TransportNotSet;
        const appDjtx = getAppDjtx(LedgerWallet.transport);
        return await appDjtx.signHash(accountPath, bip32Paths, hash);
    }
    ...
    >(unsignedTx: UnsignedTx, paths: string[], chainId: ChainIdType): Promise<SignedTx> {
        ...
        const accountPathSource = chainId === 'U' ? ETH_ACCOUNT_PATH : getAccountPathDijets(this.accountIndex);
        const accountPath = bippath.fromString(accountPathSource);
        let sigMap = await appDjtx.signHash(accountPath, bip32Paths, msg);
    ...
    async signP(unsignedTx: PlatformUnsignedTx): Promise<PlatformTx> {
        ...
        let tx = unsignedTx.getTransaction();
        let txType = tx.getTxType();
        let chainId: ChainIdType = 'M';
        let parseableTxs = ParseablePlatformEnum;

        let { paths, isDjtxOnly } = await this.getTransactionPaths<PlatformUnsignedTx>(unsignedTx, chainId);
    ...
    async signC(unsignedTx: EVMUnsignedTx): Promise<EVMTx> {
        ...
        let txSigned;
        if (canLedgerParse) {
            txSigned = (await this.signTransactionParsable(unsignedTx, paths, 'U')) as EVMTx;
        } else {
            txSigned = (await this.signTransactionHash(unsignedTx, paths, 'U')) as EVMTx;
        }
