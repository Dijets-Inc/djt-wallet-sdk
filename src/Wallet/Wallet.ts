...
import { BN, Buffer } from 'dijets';
import { FeeMarketEIP1559Transaction, Transaction } from '@ethereumjs/tx';
import { activeNetwork, dijets, cChain, pChain, web3, xChain } from '@/Network/network';
...
import {
    ...
    PlatformVMConstants,
    StakeableLockOut,
} from 'dijets/dist/apis/platformvm';
import { UnsignedTx as EVMUnsignedTx, Tx as EVMTx, UTXOSet as EVMUTXOSet } from 'dijets/dist/apis/evm';

import { PayloadBase, UnixNow } from 'dijets/dist/utils';
import { getAssetDescription } from '@/Asset/Assets';
import { getErc20Token } from '@/Asset/Erc20';
import { NO_NETWORK } from '@/errors';
import { djtxCtoX, bnToLocaleString, getTxFeeP, getTxFeeX, waitTxC, waitTxEvm, waitTxP, waitTxX } from '@/utils';
...
import { GetStakeResponse } from 'dijets/dist/apis/platformvm/interfaces';
...
export abstract class WalletProvider {
    ...
    /**
     * Returns the BTC address of the Utility-Chain public key.
     */
    ...
    /**
     * Sends DJTX to another address on the C chain using legacy transaction format.
    ...
    /**
     * Send Dijets Native Tokens on X chain
     ...
     */
    public getUsableDjtxBalanceForChain(chainType: ChainIdType, atomicFeeXP: BN, atomicFeeC: BN): BN {
     ...
     *
     * @param amt amount of nDJTX to transfer. Fees excluded.
    ...
    async exportPChain(amt: BN, destinationChain: ExportChainsP) {
        ...
        const destinationAddr = destinationChain === 'V' ? this.getAddressX() : this.getEvmAddressBech();
     ...
     * @return BN C chain atomic export transaction fee in nDJTX.
     */
    estimateAtomicFeeExportC(destinationChain: ExportChainsC, baseFee: BN): BN {
        let destinationAddr = destinationChain === 'V' ? this.getAddressX() : this.getAddressP();
        ...
        return djtxCtoX(baseFee.mul(new BN(gas)));
    }
     ...
     * @param destinationChain either `X` or `P`
     * @param exportFee Export fee in nDJTX
     * @return returns the transaction id.
     */
    async exportCChain(amt: BN, destinationChain: ExportChainsC, exportFee?: BN): Promise<string> {
        ...
        if (!exportFee) {
            ...
            exportFee = djtxCtoX(baseFee.mul(new BN(gas)));
        }
    ...
    async exportXChain(amt: BN, destinationChain: ExportChainsX) {
        let destinationAddr = destinationChain === 'M' ? this.getAddressP() : this.getEvmAddressBech();
    ...
    /**
     * Fetches Value-Chain atomic utxos from all source networks and returns them as one set.
     */
    async getAllAtomicUTXOsX() {
        const utxos = await Promise.all([this.getAtomicUTXOsX('M'), this.getAtomicUTXOsX('U')]);
        return utxos[0].merge(utxos[1]);
    }

    /**
     * Fetches Method-Chain atomic utxos from all source networks and returns them as one set.
     */
    async getAllAtomicUTXOsP() {
        const utxos = await Promise.all([this.getAtomicUTXOsP('V'), this.getAtomicUTXOsP('U')]);
        return utxos[0].merge(utxos[1]);
    }

    /**
     * Fetches Utility-Chain atomic utxos from all source networks and returns them as one set.
     */
    async getAllAtomicUTXOsC() {
        const utxos = await Promise.all([this.getAtomicUTXOsC('V'), this.getAtomicUTXOsC('M')]);
        return utxos[0].merge(utxos[1]);
    }
    ...
    async importX(sourceChain: ExportChainsX) {
        ...
        let utxoAddrs = utxoSet.getAddresses().map((addr) => bintools.addressToString(hrp, 'V', addr));
    ...
    async importP(sourceChain: ExportChainsP, toAddress?: string): Promise<string> {
        ...
        let hrp = dijets.getHRP();
        let utxoAddrs = utxoSet.getAddresses().map((addr) => bintools.addressToString(hrp, 'M', addr));
    ...
    async importC(sourceChain: ExportChainsC, fee?: BN, utxoSet?: EVMUTXOSet) {
        ...
        if (!fee) {
            ...
            const baseFee = await getBaseFeeRecommended();
            fee = djtxCtoX(baseFee.mul(new BN(importGas)));
        }
    ...
    public async issueUniversalTx(tx: UniversalTx): Promise<string> {
        switch (tx.action) {
            case 'export_x_c':
                return await this.exportXChain(tx.amount, 'U');
            case 'import_x_c':
                return await this.importC('V', tx.fee);
            case 'export_x_p':
                return await this.exportXChain(tx.amount, 'M');
            case 'import_x_p':
                return await this.importP('V');
            case 'export_c_x':
                return await this.exportCChain(tx.amount, 'V', tx.fee);
            case 'import_c_x':
                return await this.importX('U');
            case 'export_c_p':
                return await this.exportCChain(tx.amount, 'M', tx.fee);
            case 'import_c_p':
                return await this.importP('U');
            case 'export_p_x':
                return await this.exportPChain(tx.amount, 'V');
            case 'import_p_x':
                return await this.importX('M');
            case 'export_p_c':
                return await this.exportPChain(tx.amount, 'U');
            case 'import_p_c':
                return await this.importC('M', tx.fee);
            default:
                throw new Error('Method not supported.');
        }
    }

    async getHistoryX(limit = 0): Promise<OrteliusDijetsTx[]> {
        let addrs = await this.getAllAddressesX();
        return await getAddressHistory(addrs, limit, xChain.getBlockchainID());
    }

    async getHistoryP(limit = 0): Promise<OrteliusDijetsTx[]> {
        let addrs = await this.getAllAddressesP();
     ...
     * @param limit
     */
    async getHistoryC(limit = 0): Promise<OrteliusDijetsTx[]> {
    ...
    async parseOrteliusTx(tx: OrteliusDijetsTx): Promise<HistoryItemType> {
