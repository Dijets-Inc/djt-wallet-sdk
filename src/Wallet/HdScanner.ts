...
import { getPreferredHRP } from 'dijets/dist/utils';
import { activeNetwork, dijets, pChain, xChain } from '@/Network/network';
import { KeyPair as AVMKeyPair, KeyChain as AVMKeyChain } from 'dijets/dist/apis/avm/keychain';
import { KeyChain as PlatformKeyChain, KeyPair as PlatformKeyPair } from 'dijets/dist/apis/platformvm';
...
export class HdScanner {
    ...
    constructor(accountKey: bip32.BIP32Interface, isInternal = true) {
        ...
        let hrp = getPreferredHRP(dijets.getNetworkID());
        this.avmAddrFactory = new AVMKeyPair(hrp, 'V');
    }
    ...
    public getAddressX() {
        return this.getAddressForIndex(this.index, 'V');
    }

    public getAddressP() {
        return this.getAddressForIndex(this.index, 'M');
    }
     ...
     */
    public async getAllAddresses(chainId: HdChainType = 'V'): Promise<string[]> {
        let upTo = this.index;
     ...
     * @param chainId Either X or P
     */
    public getAllAddressesSync(chainId: HdChainType = 'V'): string[] {
        let upTo = this.index;
     ...
     */
    public async getAddressesInRange(start: number, end: number, chainId: HdChainType = 'V'): Promise<string[]> {
        let res = [];
     ...
     * @param chainId  `X` or `P` optional, returns X by default
     */
    public getAddressesInRangeSync(start: number, end: number, chainId: HdChainType = 'V'): string[] {
        let res = [];
        for (let i = start; i < end; i++) {
            res.push(this.getAddressForIndex(i, chainId));
        }
    ...
    public getAddressForIndex(index: number, chainId: HdChainType = 'V'): string {
    ...
    private async findAvailableIndexNode(start = 0): Promise<number> {
        ...
        for (let i: number = start; i < start + SCAN_SIZE; i++) {
            let addressX = this.getAddressForIndex(i, 'V');
            let addressP = this.getAddressForIndex(i, 'M');
        ...
        for (let i: number = 0; i < addrsX.length - HD_SCAN_GAP_SIZE; i++) {
            ...
            for (let n: number = 0; n < HD_SCAN_GAP_SIZE; n++) {
                ...
                let addrBuf = bintools.parseAddress(addr, 'V');
